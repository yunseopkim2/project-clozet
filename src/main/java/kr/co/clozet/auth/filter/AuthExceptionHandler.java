 ...
 */
@Slf4j //lombok log
@ControllerAdvice // 컨트롤러로 들어가지 직전에
public class AuthExceptionHandler {
    @ExceptionHandler(RuntimeException.class)
    protected ResponseEntity<Messenger> handleRuntimException(RuntimeException e){
        log.info("handleRuntimException", e);

        return new ResponseEntity<>(Messenger.builder().code("test")
                .message(e.getMessage()).status(HttpStatus.INTERNAL_SERVER_ERROR.value())
                .build(),
